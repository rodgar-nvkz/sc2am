...
class TemporalEncoderSequence(nn.Module):
    """Wrapper for processing full sequences through temporal encoder.

    This is used during training when we have complete episodes and want
    to process them efficiently in one pass rather than step-by-step.
    """

    def __init__(self, temporal_encoder: TemporalEncoder):
        super().__init__()
        self.encoder = temporal_encoder
        self.config = temporal_encoder.config

    def forward(
        self,
        marine_emb_seq: Tensor,
        enemy_embs_seq: Tensor,
        hidden: tuple[Tensor, Tensor] | None = None,
        enemy_mask_seq: Tensor | None = None,
    ) -> tuple[Tensor, Tensor, tuple[Tensor, Tensor]]:
        """Process full sequence through temporal GRUs.

        More efficient than calling step-by-step when we have the full
        sequence available (e.g., during training).

        Args:
            marine_emb_seq: Marine embeddings (B, T, entity_embed_size)
            enemy_embs_seq: Enemy embeddings (B, T, N, entity_embed_size)
            hidden: Initial hidden state. If None, uses zeros.
            enemy_mask_seq: Optional mask (B, T, N) where True = valid

        Returns:
            Tuple of:
                - h_marine_seq: (B, T, gru_hidden_size)
                - h_enemies_seq: (B, T, N, gru_hidden_size)
                - final_hidden: Hidden state after last timestep
        """
        batch_size, seq_len, entity_size = marine_emb_seq.shape
        num_enemies = enemy_embs_seq.shape[2]

        # Initialize hidden if not provided
        if hidden is None:
            hidden = self.encoder.get_initial_hidden(
                batch_size, num_enemies, marine_emb_seq.device
            )

        h_marine_prev, h_enemies_prev = hidden

        # === Marine GRU (full sequence) ===
        marine_out, h_marine_final = self.encoder.marine_gru(marine_emb_seq, h_marine_prev)
        # marine_out: (B, T, hidden_size)

        # === Enemy GRU (full sequence, shared weights) ===
        # Reshape: (B, T, N, d) -> (B * N, T, d)
        enemy_input = enemy_embs_seq.permute(0, 2, 1, 3)  # (B, N, T, d)
        enemy_input = enemy_input.reshape(batch_size * num_enemies, seq_len, -1)

        # Reshape hidden: (layers, B, N, d) -> (layers, B * N, d)
        h_enemies_prev_flat = h_enemies_prev.view(
            self.config.gru_num_layers,
            batch_size * num_enemies,
            self.config.gru_hidden_size,
        ...
        enemy_out, h_enemies_final_flat = self.encoder.enemy_gru(
            enemy_input, h_enemies_prev_flat
        )

        # Reshape back: (B * N, T, d) -> (B, T, N, d)
        enemy_out = enemy_out.view(batch_size, num_enemies, seq_len, -1)
        enemy_out = enemy_out.permute(0, 2, 1, 3)  # (B, T, N, d)

        h_enemies_final = h_enemies_final_flat.view(
            self.config.gru_num_layers,
            batch_size,
            num_enemies,
            self.config.gru_hidden_size,
        ...
        if enemy_mask_seq is not None:
            mask_expanded = enemy_mask_seq.unsqueeze(-1).float()  # (B, T, N, 1)
            enemy_out = enemy_out * mask_expanded

        return marine_out, enemy_out, (h_marine_final, h_enemies_final)
