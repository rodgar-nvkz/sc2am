"""
Training script for Marine vs Zergling RL environment using PPO.
...
Usage:
    python -m smarte.research.train_marine train --steps 100000 --num-envs 10
    python -m smarte.research.train_marine eval --games 10
"""
...
from smarte.envs.gym_v0 import SC2GymEnv
...
def train(steps: int = 100_000, seed: int = 42, num_envs: int = 10, resume: str | None = None):
    ...
    model_name = f"artifacts/models/marine_ppo_{time.strftime('%Y%m%d-%H%M%S')}"
...
def eval(num_games: int = 10, model_path: str | None = None):
    ...
    if model_path is None:
        try:
            model_path = max(
                glob.glob("artifacts/models/marine_ppo_*.zip"), key=os.path.getctime
            )
...
def demo():
    ...
    env = SC2GymEnv()
    print("Environment: SC2GymEnv (Marine vs Zergling)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train/evaluate Marine vs Zergling RL agent"
    )
